...
const PLANOS_VALIDOS = ["free", "basico", "pro"];
const STATUS_COMERCIAIS_VALIDOS = ["demo", "teste", "ativo", "inativo", "inadimplente", "bloqueado"];
...
function normalizePlan(value) {
  ...
  return PLANOS_VALIDOS.includes(v) ? v : "free";
}
function normalizeCommercialStatus(value) {
  const v = String(value || "ativo").trim().toLowerCase();
  return STATUS_COMERCIAIS_VALIDOS.includes(v) ? v : "ativo";
}
...
function sanitizeDate(value) {
  ...
  if (!v) return "";
  if (!/^\d{4}-\d{2}-\d{2}$/.test(v)) throw new HttpsError("invalid-argument", "Data inválida. Use AAAA-MM-DD.");
  return v;
}
function sanitizeRoomGroups(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
  return value;
}
function buildSanitizedMetaPatch(rawPatch = {}, actorRole = "admin") {
  const patch = rawPatch && typeof rawPatch === "object" ? rawPatch : {};
  const allowedAdmin = new Set([
    "open",
    "roomName",
    "roomSubtitle",
    "team1Name",
  ...
  const allowedDeveloper = new Set([
    ...allowedAdmin,
    "plan",
    "commercialStatus",
    "trialEndsAt",
  ...
  const allowed = actorRole === "developer" ? allowedDeveloper : allowedAdmin;
  const out = {};
  for (const [key, value] of Object.entries(patch)) {
    if (!allowed.has(key)) {
      throw new HttpsError("permission-denied", `Campo não permitido para alteração: ${key}`);
    }
    if (key === "open" || key === "archived" || key === "lifetime") out[key] = !!value;
    else if (key === "plan") out[key] = normalizePlan(value);
    else if (key === "commercialStatus") out[key] = normalizeCommercialStatus(value);
    else if (["trialEndsAt", "paidUntil"].includes(key)) out[key] = sanitizeDate(value);
    else if (key === "paidStartedAt" || key === "archivedAt") out[key] = sanitizeString(value, 40);
    else if (key === "closeBeforeMin") out[key] = Math.max(0, Math.min(240, Number(value) || 0));
    else if (key === "roomGroups") out[key] = sanitizeRoomGroups(value);
    else if (key === "clientNotes") out[key] = sanitizeString(value, 2000);
    else if (key === "themeColor") out[key] = /^#[0-9a-fA-F]{6}$/.test(String(value || "")) ? String(value) : "#2563eb";
    else out[key] = sanitizeString(value, 160);
  }
...
function hashAdminPassword(password, salt = makeSalt()) {
  const hash = crypto.scryptSync(String(password), salt, 64).toString("hex");
  return { adminPassHash: hash, adminPassSalt: salt, adminPassAlgo: "scrypt" };
}
...
async function migrateRoomAdminPasswordIfNeeded(code, roomSnap, plainPassword) {
  ...
  // Compatibilidade: a V6 gravava a senha admin gerada nos campos genéricos passwordHash/passwordSalt.
  if (data.passwordHash && data.passwordSalt) {
    const ok = verifyPassword(String(plainPassword || ""), data.passwordHash, data.passwordSalt);
    ...
    await matchRef(code).set({
      ...
      adminPassSalt: data.passwordSalt,
      adminPassAlgo: data.passwordAlgo || "scrypt",
      passwordHash: FieldValue.delete(),
      passwordSalt: FieldValue.delete(),
      passwordAlgo: FieldValue.delete(),
      securityVersion: 2,
  ...
  const hashed = hashAdminPassword(legacy);
  await matchRef(code).set({
    ...hashed,
    securityVersion: 2,
    adminPassUpdatedAt: nowIso(),
...
exports.createRoom = onCall(async (request) => {
  ...
  batch.set(matchRef(code), {
    code,
    open: true,
    sport: String(request.data?.sport || "futebol").trim() || "futebol",
    teamMax: Math.max(2, Math.min(15, Number(request.data?.teamMax || 12) || 12)),
    plan: "free",
...
exports.updateRoomMeta = onCall(async (request) => {
  ...
  const sanitized = buildSanitizedMetaPatch(request.data?.patch || {}, actor.role);
  await matchRef(code).set(sanitized, { merge: true });
  await appendAudit(code, "room_meta_update", { keys: Object.keys(sanitized).filter((k) => k !== "updatedAt") }, actor);
...
exports.adminEnterRoom = onCall(async (request) => {
  ...
  } else if (room.passwordHash && room.passwordSalt) {
    // Compatibilidade com salas da V6, onde a senha admin gerada foi gravada como passwordHash/passwordSalt.
    ok = verifyPassword(password, room.passwordHash, room.passwordSalt);
  } else {
...
exports.migrateRoomSecurity = onCall(async (request) => {
  ...
  let migratedRoom = 0;
  const room = roomSnap.data() || {};
  if (!room.adminPassHash && room.passwordHash && room.passwordSalt) {
    await matchRef(code).set({
      adminPassHash: room.passwordHash,
      adminPassSalt: room.passwordSalt,
      adminPassAlgo: room.passwordAlgo || "scrypt",
      passwordHash: FieldValue.delete(),
      passwordSalt: FieldValue.delete(),
      passwordAlgo: FieldValue.delete(),
      securityVersion: 2,
    ...
    migratedRoom = 1;
  } else if (!room.adminPassHash && room.adminPass) {
    const hashed = hashAdminPassword(String(room.adminPass || "").trim() || DEFAULT_ADMIN_PASSWORD);
    await matchRef(code).set({
      ...hashed,
      securityVersion: 2,
      adminPassUpdatedAt: nowIso(),
