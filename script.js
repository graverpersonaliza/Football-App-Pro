    ...
    // ===============================
    const firebaseConfig = window.FOOTBALL_APP_FIREBASE_CONFIG || {};

    // ===============================
    // Config do app
    // ===============================
    const LS_SESSION = "mfut_session_v1";
    const LS_LAST_CODE = "mfut_last_code_v1";
    const LS_GROUPS = "mfut_groups_v1";
    const LS_NOTIFY_PREF = "mfut_notify_pref_v1";
    const LS_DEVICE_ID = "mfut_device_id_v1";
    const LS_ROOM_PLAYER_MAP = "mfut_room_player_map_v1";
    ...
    // Limite de jogadores por time (futebol: 12 por time)
    const TEAM_MAX = 12;
    ...
    const APP_VERSION = "2.0.3-football-app-v6";
    const APP_BUILD = "2026-04-25-football-app-v6-create-room-dev-fix";
    const APP_NAME = "Football App";
...
function evaluationReminderText(){
  ...
  if(!evaluatorCount) return `Ainda ninguém avaliou a último jogo. Faça pelo menos ${Math.max(1, Number(evaluationInsights.requiredCount || 0))} avaliações para liberar sua próxima inscrição.`;
  if(evaluatorCount <= 2) return `Somente ${evaluatorCount} jogador${evaluatorCount === 1 ? '' : 'es'} avaliaram a último jogo. Faça suas avaliações para ajudar a equilibrar os próximos times.`;
...
async function loginPlayerByNamePassword(){
  ...
  try{
    ...
    if(!code) return alert("Entre na sala primeiro (código do jogo).");
    await ensureFirebaseClientReady();
  ...
  return e.replace(/&lt;b&gt;/g, "<b>").replace(/&lt;\/b&gt;/g, "</b>");
    ...
    async function enableSystemNotifications(){
      try{
        ...
        if(allowed){
          await pushSystemNotification("Football App", "Notificações ativadas para inscrições, presença e escolha de time.", { force:true, tag:"notif-onboarding" });
    ...
    function accessBadgeHtml(){
      ...
      if(mode === "developer") return `<span class="text-xs px-2 py-1 rounded-full bg-amber-100 text-amber-800">Acesso avançado</span>`;
    ...
    function upgradeWhatsLink(feature){
      const msg = encodeURIComponent(`Olá! Quero liberar ${feature || "os recursos premium"} no Football App.`);
    ...
    async function saveCurrentInternalGroup(){
      ...
      if(!session.admin && !session.developer){
        setInfo('Somente Admin podem salvar grupos internos.');
        return false;
      }
    ...
    async function updateCurrentInternalGroup(){
      ...
      if(!session.admin && !session.developer){
        setInfo('Somente Admin podem atualizar grupos.');
        return false;
      }
      const roomGroups = Object.assign({}, state.roomGroups || {});
    ...
    async function openInternalGroup(groupId){
      ...
      try{
        await metaUpdate(code, {
          roomName: String(group.roomName || state.roomName || '').trim() || 'Football App',
    ...
    async function deleteInternalGroup(groupId){
      ...
      if(!code || !group) return false;
      if(!session.admin && !session.developer){
        setInfo('Somente Admin podem excluir grupos internos.');
        return false;
      }
    ...
    function roomRestrictionMessage(meta = state, mode = accessMode()){
      ...
      if(status === "inadimplente") return "Esta sala está bloqueada por inadimplência. Fale com o administrador para regularizar o acesso.";
      if(status === "bloqueado") return "Esta sala está temporariamente bloqueada pelo administrador.";
      ...
      if(status === "ativo" && meta.paidUntil){
        ...
        if(paidUntil){
          ...
          if(Date.now() > endMs) return `A mensalidade desta sala venceu em ${fmtDatePt(paidUntil)}. Fale com o administrador para renovar o acesso.`;
        }
      }
      return "";
    }
    ...
    function commercialAlertInfo(meta = state){
      ...
      if(status === "bloqueado") return { level:"danger", kind:"bloqueado", text:"Sala bloqueada pelo administrador." };
    ...
    function commercialAlertBoxHtml(meta = state){
      ...
      ` : "";
      return `<div class="mt-4 p-3 rounded-xl border ${styles}"><div class="font-semibold">Aviso da sala</div><div class="text-sm mt-1">${escapeHtml(alert.text)}</div>${devActions}</div>`;
    }
...
async function saveMatchSchedule(){
  if(!session.admin) return alert("Somente admin.");
  const code = String(state.code||"").toUpperCase();
  if(!code) return alert("Entre em uma sala primeiro (código do jogo).");
  ...
  if(!matchDate || !matchTime) return alert("Informe a data e a hora da jogo.");
...
async function resolveExistingRegistrationForThisDevice(code){
  ...
  return null;
    ...
    function topPosition(players){
      const count = {};
      (players || []).forEach(p=>{
        const pos = String((p && p.position) || "Meia");
    ...
    function buildManagementReport(players, presentPlayers, waitingPlayers, team1Players, team2Players, byTarget){
      ...
      const lines = [
        `RELATÓRIO · ${state.roomName || "Football App"}`,
    ...
    async function savePersonalization(){
      ...
      try{
        const roomName = String(document.getElementById("roomNameInput")?.value || "").trim() || "Football App";
    ...
    function quickStartText(){
      return [
        ...
        `7) O Admin acompanha jogadores, presença, times e histórico.`
      ].join("\n");
    }
    ...
    async function downloadRoomBackup(){
      if(!session.admin && !session.developer) return alert("Somente Admin podem baixar o backup.");
      try{
    ...
    async function importRoomBackupObject(payload){
      if(!session.admin && !session.developer) return alert("Somente Admin podem restaurar backup.");
    ...
    async function downloadAuditLog(){
      if(!session.admin && !session.developer) return alert("Somente Admin podem baixar o log.");
      try{
        const c = normalizeRoomCode(state.code);
      ...
      return `
        <div class="mt-5 rounded-2xl border bg-white p-4">
          ...
          <div class="mt-3 grid gap-3 md:grid-cols-3 text-sm">
            ...
            <div class="rounded-xl border bg-gray-50 p-3"><div class="font-semibold text-gray-800">Fase 2</div><div class="mt-1 text-xs text-gray-600">Operação da jogo, cadastro, presença, times, histórico e exportações.</div></div>
            <div class="rounded-xl border bg-gray-50 p-3"><div class="font-semibold text-gray-800">Fase 3 e 4</div><div class="mt-1 text-xs text-gray-600">Relatórios, ajuda, documentação e escala.</div></div>
          </div>
        </div>
      `;
    }
    ...
    async function developerSetCommercialStatus(code, status){
      ...
      try{
        ...
        await loadDeveloperRooms(false);
        setInfo(`Status da sala ${safeCode} ajustado para ${commercialStatusLabel(nextStatus)}.`);
        render();
      }catch(e){
        setSyncError(e && e.message ? e.message : String(e || "Erro ao aplicar status da sala."));
      }
    }
    ...
    <div class="mt-5 rounded-2xl border bg-amber-50 p-4">
      <div class="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <p class="text-xs text-gray-600">Gestão central de salas, planos, responsáveis e suporte interno.</p>
          <div class="mt-2 flex flex-wrap gap-2 text-[11px]">
            ...
            <span class="px-2 py-1 rounded-full bg-red-100 text-red-700">Pendentes ${inadCount}</span>
        ...
        <div class="flex flex-wrap gap-2">
          <button id="btnCreateTrialRoom" onclick="createFreeTrialRoom()" type="button" class="px-3 py-2 rounded-lg bg-sky-600 text-white hover:bg-sky-700 text-sm font-semibold">Criar sala de teste</button>
...
Edite qualquer sala por aqui. Use a limpeza de salas inativas ou órfãs com cuidado.
        </div>
          ...
          return `
            <div class="rounded-xl border bg-white p-3">
              ...
              <div class="mt-2 grid gap-2 md:grid-cols-3">
                ...
                <input data-dev-owner="${escapeHtml(room.code)}" value="${escapeHtml(room.ownerName || '')}" placeholder="Responsável" class="px-2.5 py-2 rounded-lg border text-xs" ${lockField} />
                <input data-dev-whats="${escapeHtml(room.code)}" value="${escapeHtml(room.ownerWhatsApp || '')}" placeholder="WhatsApp do responsável" class="px-2.5 py-2 rounded-lg border text-xs" ${lockField} />
              </div>
              <div class="mt-2 grid gap-2 md:grid-cols-3">
              ...
              <div class="mt-2 grid gap-2 md:grid-cols-4">
                ...
                <label class="block">
                  <span class="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-gray-500">Status da sala</span>
                  <select data-dev-status="${escapeHtml(room.code)}" class="w-full px-2.5 py-2 rounded-lg border text-xs" ${lockField}>
                    ...
                    <option value="inadimplente" ${normalizeCommercialStatus(room.commercialStatus)==='inadimplente' ? 'selected' : ''}>Pendente</option>
                    <option value="bloqueado" ${normalizeCommercialStatus(room.commercialStatus)==='bloqueado' ? 'selected' : ''}>Bloqueado</option>
                    <option value="demo" ${normalizeCommercialStatus(room.commercialStatus)==='demo' ? 'selected' : ''}>Demo</option>
                  </select>
                </label>
                <label class="block">
                  <span class="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-gray-500">Fim do teste</span>
              ...
              <textarea data-dev-notes="${escapeHtml(room.code)}" placeholder="Observações internas e suporte" class="mt-2 w-full px-2.5 py-2 rounded-lg border text-xs min-h-[64px]" ${lockField}>${escapeHtml(room.clientNotes || '')}</textarea>
              <div class="mt-2 flex flex-wrap gap-1.5">
                ...
                <button data-dev-copyclient="${escapeHtml(room.code)}" onclick="developerCopyClientSummary('${escapeHtml(room.code)}')" type="button" class="px-2 py-1.5 rounded-lg border hover:bg-gray-50 text-[11px] font-semibold">Copiar dados</button>
  ...
  `;
    ...
    async function createRoom(){
      try{
        ...
        await appendAuditLog("room_create", { roomCode: code, source: session.developer ? "developer" : "admin", sport: "futebol", teamMax: TEAM_MAX }, code).catch(()=>{});
        setInfo("Sala criada. Compartilhe o código ou o link da sala para os jogadores entrarem.");
    ...
    function openDemoRoom(){
      ...
      setInfo("A demonstração visual fica no site inicial. Para testar de verdade, use 'Criar teste grátis' ou entre com um código válido.");
      render();
    }
    ...
    async function joinRoomByCode(rawCode){
      try{
        ...
        const entryMsg = mode === "developer"
          ? "<b>Entrou na sala como Admin avançado.</b> Você tem visão e controle avançado desta sala."
          : mode === "admin"
    ...
    async function createPlayerByAdmin(opts = {}){
      const code = state.code;
      if(!code) throw new Error("Entre em uma sala primeiro (código do jogo).");
...
async function markPresence(present){
      ...
      if(evaluationReminderActive()) return alert(`Antes da nova inscrição, faça pelo menos ${Math.max(1, Number(evaluationInsights.requiredCount || 0))} avaliações da último jogo.`);
    ...
    async function chooseTeam(team){
      try{
        ...
        if(evaluationReminderActive()) return alert(`Antes de escolher time, faça pelo menos ${Math.max(1, Number(evaluationInsights.requiredCount || 0))} avaliações da último jogo.`);
        ...
        // Limite do futebol: máximo de 12 por time; acima de 24 distribuídos, extras ficam em espera
        ...
        // Se você ainda não tem time e já existem 14 jogadores distribuídos, você fica em espera
        if (currentTeam == null && totalAssignedExcludingMe >= TEAM_MAX*2) {
          return alert("Os times já estão completos. Você ficará em espera.");
        }

        // Checa lotação do time alvo (considerando que você vai entrar nele)
        if (team === 1 && count1 >= TEAM_MAX) {
          return alert("Time 1 já está completo (12 jogadores).");
        }
        if (team === 2 && count2 >= TEAM_MAX) {
          return alert("Time 2 já está completo (12 jogadores).");
        }
...
Continuar?`);
    ...
    async function randomizeTeams(){
      ...
      // Seleciona no máximo 24 jogadores (12 por time). Os demais ficam em espera (team null)
      ...
      // Aplica times aos 24 selecionados
      ...
      await batch.commit();
      setInfo("Times sorteados. Jogadores extras ficaram em espera.");
    }
    ...
    function openRatingModal(){
      ...
      // ✅ lista todos os cadastrados (exceto você), priorizando pendências da último jogo
  ...
  window.open(base + encodeURIComponent(text), "_blank");
    ...
    function buildMiniListExportElement(payload){
      ...
      const maxTeamRows = Math.max(team1Rows.length, team2Rows.length, 1);
      const dynamicHeight = Math.max(920, 520 + (maxTeamRows * 60));
      const schedule = getMatchScheduleMeta();
      const dateParts = [];
      if(state.matchLocation) dateParts.push(`📍 ${escapeHtml(state.matchLocation)}`);
      if(state.matchDate) dateParts.push(`📅 ${escapeHtml(state.matchDate.split('-').reverse().join('/'))}`);
      if(state.matchTime) dateParts.push(`🕒 ${escapeHtml(state.matchTime.replace(':','h'))}`);
      else if(schedule && schedule.hasSchedule && schedule.matchLabel){
        const m = schedule.matchLabel.split(',');
        if(m[1]) dateParts.push(`🕒 ${escapeHtml(m[1].trim().slice(0,5).replace(':','h'))}`);
      }

      wrap.style.width = '1080px';
      wrap.style.minHeight = dynamicHeight + 'px';
      wrap.style.background = '#f6f7fb';
      wrap.style.padding = '28px';
      wrap.style.borderRadius = '34px';
      wrap.style.fontFamily = 'system-ui,-apple-system,Segoe UI,Roboto,Arial';
      wrap.style.color = '#0f172a';
      wrap.style.boxSizing = 'border-box';
      wrap.style.border = '10px solid #f3f4f6';

      function buildRosterRows(list){
        return list.map((row, idx)=>`
          <div style="display:grid;grid-template-columns:108px 1fr;align-items:center;min-height:70px;border-bottom:${idx === list.length - 1 ? '0' : '2px solid #dbe3f1'};">
            <div style="display:flex;align-items:center;justify-content:center;height:100%;background:#facc15;color:#0b2a8d;font-size:34px;font-weight:950;border-right:2px solid #dbe3f1;">${escapeHtml(String(row.number || ''))}</div>
            <div style="padding:0 26px;font-size:32px;font-weight:900;color:#11206f;line-height:1.05;">${escapeHtml(row.name || '')}</div>
          </div>
        `).join('') || `<div style="padding:26px;font-size:26px;font-weight:700;color:#64748b;">Sem jogadores neste bloco.</div>`;
      }

      wrap.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;text-align:center;gap:16px;">
          <img src="./icon-512.png" alt="Football App" style="width:300px;height:300px;object-fit:contain;display:block;">
          <div style="font-size:84px;font-weight:1000;letter-spacing:0.04em;line-height:0.95;color:#0b2a8d;text-transform:uppercase;">Mini lista da rodada</div>
          <div style="display:flex;align-items:center;justify-content:center;flex-wrap:wrap;gap:24px;padding:14px 26px;border:2px solid #263b86;border-radius:22px;background:#fff;font-size:26px;color:#1f2b5b;min-width:920px;">
            ${dateParts.join('<span style="color:#334155;">•</span>') || `<span><b>Código:</b> ${escapeHtml(payload.code || '')}</span>`}
          </div>
        </div>

        <div style="display:grid;grid-template-columns:1fr 1fr;gap:34px;margin-top:28px;align-items:start;">
          <div style="background:#fff;border-radius:24px;overflow:hidden;box-shadow:0 10px 28px rgba(15,23,42,0.12);border:2px solid #e5e7eb;">
            <div style="background:#0b2a8d;color:#facc15;text-align:center;font-size:70px;font-weight:1000;letter-spacing:0.03em;padding:14px 18px;text-transform:uppercase;">${escapeHtml(team1Label)}</div>
            ${buildRosterRows(team1Rows)}
          </div>
          <div style="background:#fff;border-radius:24px;overflow:hidden;box-shadow:0 10px 28px rgba(15,23,42,0.12);border:2px solid #e5e7eb;">
            <div style="background:#0b2a8d;color:#facc15;text-align:center;font-size:70px;font-weight:1000;letter-spacing:0.03em;padding:14px 18px;text-transform:uppercase;">${escapeHtml(team2Label)}</div>
            ${buildRosterRows(team2Rows)}
          </div>
        </div>

        <div style="margin-top:34px;background:#0b2a8d;color:#fff;border-radius:24px;padding:18px 28px;display:flex;align-items:center;gap:18px;box-shadow:0 10px 28px rgba(15,23,42,0.14);">
          <div style="font-size:34px;line-height:1;">👥</div>
          <div style="font-size:34px;font-weight:1000;color:#facc15;text-transform:uppercase;letter-spacing:0.02em;white-space:nowrap;">Aguardando:</div>
          <div style="font-size:26px;font-weight:800;line-height:1.2;">${waitingRows.length ? waitingRows.map(r => `${escapeHtml(String(r.number || ''))} ${escapeHtml(r.name || '')}`).join(' <span style="color:#facc15;">•</span> ') : 'Nenhum inscrito aguardando'}</div>
        </div>
      `;
    ...
    function buildExportElement(payload){
      ...
      wrap.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:16px;">
          <div style="font-size:30px;font-weight:950;">⚽ Football App</div>
          <div style="text-align:right;font-size:14px;color:#374151;">
            <div><b>Código:</b> ${escapeHtml(code)}</div>
            ${matchLabel ? `<div><b>Jogo:</b> ${escapeHtml(matchLabel)}</div>` : ``}
      ...
      return `
        <div class="mt-6 rounded-2xl border bg-gray-50 p-4">
          ...
          <div class="mt-4 grid gap-3 sm:grid-cols-3">
            ...
            <div class="rounded-2xl border bg-amber-50 p-4">
              <div class="text-sm font-bold text-amber-800">Desenvolvedor</div>
              <p class="mt-1 text-xs text-amber-800">Acesso avançado para controlar salas, planos e suporte interno.</p>
              <input id="homeDeveloperPass" type="password" placeholder="Senha do desenvolvedor" class="mt-3 w-full px-3 py-2 rounded-lg border bg-white" />
        ...
        <div class="mt-6 grid gap-3 sm:grid-cols-3">
          <div class="flex gap-2 sm:col-span-2">
            <input id="roomCode" placeholder="Código da jogo (ex.: A2K9ZP)" autocapitalize="characters" autocomplete="off" autocorrect="off" spellcheck="false" class="flex-1 px-3 py-2 rounded-lg border" />
        ...
        <div class="mt-3 text-sm text-gray-600">
          Dica: use sempre o mesmo código da sala para manter as notas de semana a semana. ${canCreate ? '' : 'A criação de nova sala fica liberada para Admin.'}
        </div>
      ...
      return `
        ...
        ${evaluationBlocked ? `
          <div class="mt-2 rounded-xl border border-violet-200 bg-violet-50 p-3 text-sm text-violet-900">
            <div class="font-extrabold">Avaliação pendente da último jogo</div>
        ...
        <div class="border-t pt-3">
          <div class="grid gap-2">
            ...
            ${evaluationBlocked ? `<div class="text-xs text-violet-700">Antes de concluir a próxima inscrição, faça pelo menos <b>${Math.max(1, Number(evaluationInsights.requiredCount || 0))}</b> avaliações da último jogo. Restam <b>${evaluationRemaining}</b>.</div>` : ``}
    ...
    function render(){
      ...
      const roomDisplayName = (state.roomName || "").trim() || "Football App";
      ...
      try{ document.title = state.code ? `${roomDisplayName} | ${String(state.code).toUpperCase()}` : "Football App"; }catch{}
      ...
      const evaluationSummaryHtml = state.lastClosedRoundId && evaluationInsights.roundId === state.lastClosedRoundId && Number(evaluationInsights.participantCount || 0) ? `
        <div class="mt-4 rounded-2xl border border-violet-200 bg-violet-50 p-4">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <div class="text-sm font-extrabold text-violet-900">Avaliação do último jogo</div>
              <div class="mt-1 text-sm text-violet-800">${Number(evaluationInsights.evaluatorCount || 0) <= 2 ? `Somente ${Number(evaluationInsights.evaluatorCount || 0)} jogador${Number(evaluationInsights.evaluatorCount || 0) === 1 ? '' : 'es'} avaliaram a último jogo.` : `${Number(evaluationInsights.evaluatorCount || 0)} jogadores já avaliaram a último jogo.`}</div>
            </div>
            ...
            <div>
              ...
              ${scheduleMeta.hasSchedule ? `
                <div class="mt-1 space-y-1 text-sm text-gray-600">
                  <div>⏰ Próxima jogo: <b>${escapeHtml(matchLabel)}</b></div>
              ...
              <div class="lg:col-span-1 bg-white rounded-2xl border p-4 space-y-3">
                ...
                <div class="border-t pt-3">
                  <h3 class="font-semibold mb-2">Compartilhar</h3>
                    ...
                    <div class="mt-2 text-xs text-gray-500">Somente admin. A mini lista gera uma figurinha simples com número, nome e time ao longo das presenças. O log registra ações críticas.</div>
                    ...
                    <div class="mt-4">
                      ...
                      ` : `
                        ${canUseHistory ? `<div class="mt-2 text-sm text-gray-500">Ainda não há times salvos.</div>` : `<div class="mt-2">${premiumLockCard("Histórico de jogos", "No plano Free o histórico fica bloqueado. Libere no Básico ou PRO.", "histórico de jogos")}</div>`}
                      `}
                    </div>
                  </div>
                ` : ``}
                ...
                <div class="border-t pt-3">
                  ${session.admin ? `
                    ...
                    <div class="mt-3 rounded-xl border p-3 bg-gray-50">
                      <div class="text-sm font-semibold">Horário do jogo</div>
                    ...
                    <div class="mt-3 rounded-xl border p-3 bg-gray-50">
                      ...
                      ${canUseCustomization ? `
                        <div class="mt-2 grid gap-2">
                          <input id="roomNameInput" type="text" maxlength="40" value="${escapeHtml(state.roomName || "Football App")}" class="px-3 py-2 rounded-lg border" placeholder="Nome do grupo/app" />
                ...
                <div class="bg-white rounded-2xl border p-4 md:col-span-2">
                  ${plan === "free" ? `
                    ...
                    <div class="mt-4 grid gap-3">
                      ${premiumLockCard("Histórico de jogos", "Salve e consulte jogos anteriores no plano Básico ou PRO.", "histórico de jogos")}
                  ...
                  ` : `
                    ...
                    <div class="mt-4 grid gap-4 xl:grid-cols-3">
                      ...
                      <div class="rounded-2xl border p-3 bg-gray-50">
                        <div class="font-semibold text-gray-800">Histórico de jogos</div>
                        <div class="mt-3 space-y-2 max-h-[360px] overflow-y-auto pr-1">
                          ...
                          `).join("") : `<div class="text-sm text-gray-500">Nenhuma jogo salva ainda.</div>`}
                        </div>
...
window.addEventListener("unhandledrejection", (ev)=>{
  try{
    ...
    setSyncError("Promise rejeitada: " + msg);
      ...
      try{
        if(!firebaseConfig || !firebaseConfig.projectId || String(firebaseConfig.projectId).includes("SEU_PROJETO") || String(firebaseConfig.apiKey || "").includes("COLE_AQUI")){
          throw new Error("firebase-config.js ainda não foi configurado com os dados do novo projeto Firebase Football App.");
        }
      ...
      auth.onAuthStateChanged(async (user)=>{
        ...
        if(!user){
          try{
            ...
            setSyncError("Falha ao autenticar no Firebase Auth. Ative Authentication > Sign-in method > Anonymous no projeto football-app-pro. Detalhe: " + (e && e.message ? e.message : e));
            markAuthReady();
            return;
          }
        }
      ...
      if(!auth.currentUser){
        auth.signInAnonymously().catch((e)=>{
          setSyncError("Falha ao iniciar Auth anônimo. Ative Authentication > Sign-in method > Anonymous no projeto football-app-pro. Detalhe: " + (e && e.message ? e.message : e));
          markAuthReady();
        });
      }else{
